...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		poseIncrement.apply(source, output);

		T target[3];
		fillVector(m_targetPoint, target);

		//loss function set to NULL already takes squared norm of residuals!
		residuals[0] =  T(LAMBDA) * T(m_weight) * (output[0] - target[0]);
		residuals[1] =  T(LAMBDA) * T(m_weight) * (output[1] - target[1]);
		residuals[2] =  T(LAMBDA) * T(m_weight) * (output[2] - target[2]);
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		residuals[0] = T(LAMBDA) * T(m_weight) * (targetNormal[0] * (output[0] - target[0]) + 
									  targetNormal[1] * (output[1] - target[1]) + 
									  targetNormal[2] * (output[2] - target[2]));
	...
	void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			if (match.idx >= 0) {
				...
				float angle = acos((sourceNormal).dot(targetNormal))*180 / M_PI;
				if (angle >= 60) {
					match = Match{ -1, 0.f };
...
private:
	...
	Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			VectorXf row2(6);
			VectorXf row3(6);
			row1 << 0, s(2), -s(1), 1.0f, 0, 0;
			row2 << -s(2), 0, s(0), 0, 1.0f, 0;
			row3 << s(1), -s(0), 0, 0, 0, 1.0f;
			A.block<1,6>(4*i+1,0) = row1 * 0.1;
			A.block<1,6>(4*i+2,0) = row2 * 0.1;
			A.block<1,6>(4*i+3,0) = row3 * 0.1;
